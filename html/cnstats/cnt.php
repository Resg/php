...
function stats_hit($sqlhost,$sqluser,$sqlpassword,$db_name) {
	GLOBAL $HTTP_SERVER_VARS,$HTTP_COOKIE_VARS,$STATS_CONF,$COUNTER,$HTTP_GET_VARS,$CNSTATS_DR;
	...
	while (list ($key, $val) = each ($COUNTER["excludeip"])) {
		...
		if ((ip2long($HTTP_SERVER_VARS["REMOTE_ADDR"])&$emask)==($eip&$emask)) {$exclude=true;break;}
	}
	if ($exclude) {
	...
	$agent=mysql_escape_string(htmlspecialchars($HTTP_SERVER_VARS["HTTP_USER_AGENT"]));
	...
	if ($flag==1) {
		...
		if (is_file($CNSTATS_DR."cnstats/geoip/GeoIPCity.dat")) {
			include($CNSTATS_DR."cnstats/geoip/geoipcity.inc");
			$gi = geoip_open($CNSTATS_DR."cnstats/geoip/GeoIPCity.dat",GEOIP_STANDARD);
		...
		elseif (is_file($CNSTATS_DR."cnstats/geoip/GeoIP.dat")) {
			include($CNSTATS_DR."cnstats/geoip/geoip.inc");
			$gi = geoip_open($CNSTATS_DR."cnstats/geoip/GeoIP.dat",GEOIP_STANDARD);
