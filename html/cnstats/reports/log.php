...
if ($HTTP_GET_VARS["users"]=="yes") $where.=" AND type1=1";
...
if ($HTTP_GET_VARS["sel_city"]==1) $where.=" AND city like '%".str_replace("+"," ",str_replace("%","\%",urlencode(cnstats_mhtml($HTTP_GET_VARS["inp_city"]))))."%'";
if ($HTTP_GET_VARS["sel_city"]==2) $where.=" AND city not like '%".str_replace("+"," ",str_replace("%","\%",urlencode(cnstats_mhtml($HTTP_GET_VARS["inp_city"]))))."%'";
...
if ($by==1) {
	...
	for ($i=$start;$i<$finish;$i++) {
	    ...
	    $num++;
		$page=phrase_uncode(urldecode($page));
		$from=phrase_uncode(urldecode($from));
